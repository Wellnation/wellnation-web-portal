...
const Home = () => {
  ...
  async function fetchtests() {
    const camps = await getDocs(query(collection(db, "campaign"), where("hid", "==", hId)))
    return camps
  }
              ...
              {!data.docs? (
                ...
                data.docs.map((tests) => (
                  <>
                    ...
                    >
                      <ListItemText
                        primary={<Typography variant="h5">{tests.data().name}</Typography>}
                        secondary={
                          <>
                            <table>
                              <tr>
                                <th>Camp starts on: </th>
                                <td>{tests.data().start.toDate().toDateString() + " at " + tests.data().start.toDate().toLocaleTimeString("en-us")}</td>
                              </tr>
                              <tr>
                                <th>Camp ends on: </th>
                                <td>{tests.data().end.toDate().toDateString() + " at " + tests.data().end.toDate().toLocaleTimeString("en-us")}</td>
                              </tr>
                            </table>
                          </>
                        }
